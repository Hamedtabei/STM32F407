...
void ST7789_Test(void)
{
	...
	ST7789_Fill_Color(WHITE);
	ST7789_DrawImage(40, 0, 240, 240, (uint16_t *)mta_240x240);
	HAL_Delay(3000);
}


void ST7789_DrawAradLogo(void){
	/* uncomment 			 mta_240x240 (or mta_320x240) array   in 	fonts.c
	 * and
	 * extern const uint16_t mta_240x240[][240];(or mta_320x240)  in 	fonts.h
	*/
	ST7789_Fill_Color(WHITE);
	if(ST7789_ROTATION == 1 ||ST7789_ROTATION == 3)
		ST7789_DrawImage(0, 0, 320, 240,(uint16_t*) mta_320x240);

	else if(ST7789_ROTATION == 0 ||ST7789_ROTATION == 2)
		ST7789_DrawImage(0, 40, 240, 240, (uint16_t *)mta_240x240);
	HAL_Delay(3000);
}

void ST7789_WriteIncrementNumbers(uint16_t n){
	char *numbers;
	uint8_t w;
	uint8_t h;
	...
	for(uint16_t i = 0; i < n; i ++){
		sprintf(numbers, "%d", i);
		ST7789_WriteString(w, h, numbers , Font_16x26,  WHITE, BLACK);
		HAL_Delay(1000);
	}
}

void ST7789_DrawBlinkCircle(uint16_t color){
	/*
	 * use in while loop or add a loop inside function instead
	 * */
	ST7789_Fill(80, 40, 240, 200, BLACK);
	ST7789_DrawCircle(160, 120, 80,  color);
	HAL_Delay(500);
	ST7789_DrawFilledCircle(160, 120, 80, color);
	HAL_Delay(500);
}
